...
def steer_to(img, p1, p2):
    ...
    def calculate_line(p1, p2):
        x0, y0 = p1.detach().cpu().numpy()
        x1, y1 = p2.detach().cpu().numpy()
        a = (y1 - y0) / (x1 - x0)
        b = y0 - a * x0

        x_range = round(abs(x0 - x1))
    ...
    return check_oversection(img.detach().cpu().numpy(), line)


def lazy_states_contraction(path, x_obs):

    can_connect = True
    while can_connect:
        for i in range(len(path) - 2):

            if steer_to(x_obs, path[0 + i], path[2 + i]):
                path.pop(i + 1)
                can_connect = False
...
def is_feasible(path, x_obs):

    feasible = True
    for i in range(len(path) - 1):
        if not steer_to(x_obs, path[0 + i], path[1 + i]):
            feasible = False
